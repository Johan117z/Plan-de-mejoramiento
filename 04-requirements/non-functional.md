# Non-Functional Requirements (NFR) - FixGo Platform
...
| ------------------------------------ | ------------ | --------------------- |
| P95 latency — critical endpoints     | < 250ms      | Under 500 RPS load    |
| P99 latency — critical endpoints     | < 450ms      | Under 500 RPS load    |
| P95 latency — non-critical endpoints | < 800ms      | Normal load (100 RPS) |
| Minimum throughput                   | 1000 RPS     | Without degradation   |
| Service startup time                 | < 15 seconds | Cold start            |

**Defined critical endpoints:**

- `POST /api/v1/assistance-requests` — Critical because drivers in emergency situations need instant confirmation.
- `GET /api/v1/mechanics/nearby` — Critical for rendering real-time geospatial markers on the active map.
...
- `GET /health/ready` — Readiness: responds 200 only if it can process traffic (PostgreSQL, Redis, and WebSocket connections active).
...
| -------------------------------- | ------------------------------------------------------------------ |
| Gradual load growth              | Horizontal auto-scaling activated when CPU > 70% or RAM > 80%      |
| Sudden spike (High demand hours) | System scales in < 2 minutes                                       |
| Load reduction                   | Scale-down without interrupting active WebSocket location sessions |
| Horizontal scaling limit         | Up to 10 instances per microservice                                |

**Strategy:** Stateless horizontal scaling — each instance does not store state in memory. State goes in Redis (sessions, active GPS sockets) or PostgreSQL/PostGIS (persistent data).
...
- RBAC (Role-Based Access Control): roles defined in `00-governance/security-policy.md` (DRIVER, MECHANIC, ADMIN)
...
- Passwords: hashing with bcrypt (cost factor ≥ 12)
- PII (personal data & precise GPS logs): encrypted at rest
...
- Habeas Data (Ley 1581 de 2012 - Colombia) for user personal data protection and location history.
...
| Traces  | End-to-end distributed traces for location events | OpenTelemetry + Jaeger |
| Alerts  | Alert in < 5 min when SLI violates SLO            | Alertmanager           |
...
| Metric                | Target                                                                        |
| --------------------- | ----------------------------------------------------------------------------- |
| Test coverage         | ≥ 80% of lines (≥ 90% in the domain layer)                                    |
...
| Security        | P1                  | Yes (SAST + OWASP)     | Security Lead    |
